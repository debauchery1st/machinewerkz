...
class Picard:
    ...
    class __Picard__:
        ...
        def __init__(self, **kwargs):
            ...
            self.block_img = self.pg.image.load('data/img/steampunk.png')
        ...
        def __draw_board(self, board=None, keeping_score=False):
            scr = []
            if board is None:
            ...
            for y in range(len(board)):
                if 0 not in board[y] and keeping_score:
                    scr.append(y)
                    # print('score this row')
                for x in range(len(board[0])):
                    if board[y][x] == 1:
                        _x, _y = x * self.board.square_unit, y * self.board.square_unit
                        surface = self.pg.draw.rect(
                            self.screen, self.color,
                            (_x, _y, self.board.square_unit, self.board.square_unit))
                        self.screen.blit(self.block_img, surface)
            if len(scr) > 0:
        ...
        def set_color(self):
            idx = (self.colors.index(self.color) + 1) % len(self.colors)
        ...
        def draw(self):
            ...
            if error != 0:
                board = self.__draw_board(board=self.swap, keeping_score=True)
                self.swap = board
                del locutus, of_borg
                if error == 3:
            ...
            if not self.overlap(of_borg, locutus, z):
                ...
                self.__draw_board(board=locutus)
                result = True, "ok"
            else:
                if self.fell < 1:
                    self.game_over()
                self.__draw_board(board=self.swap, keeping_score=True)
...
class Borg:
    ...
    def __init__(self, **kwargs):
        ...
        for prim in kwargs.keys():
            self.__setattr__(prim, kwargs[prim])
